...
5. [Claude Code Hooks: Sentinel's Automated Guardrails](#5-claude-code-hooks-sentinels-automated-guardrails)
...
- **Skill `record-eval-baseline`** — automates §8.5 DoD item 4 ("recorded as a new
  baseline in the Feature Log, not just observed and discarded"), but its most
  important behavior is what it refuses to do: `scripts/run_eval.py` today only
  validates harness mechanics and explicitly says so in its own output ("No quality
  baseline yet...") — the skill greps for that exact honesty marker and, if
  present, **stops and reports a no-op rather than writing a fabricated score to the
  PMA.** This is the same "never silently default" discipline as
  `EmbeddingDimensionMismatchError` (§3.5) and every AP-0N anti-pattern below,
  applied to documentation instead of code. `disable-model-invocation: true`
  (user-only) since it runs real model calls and shouldn't fire unprompted.
- **Subagent `gherkin-convention-checker`** — the Gherkin-side counterpart to
  `adr-conflict-checker`: enforces §8.3's tagging rules mechanically, most
  importantly that every `@hitl`/`@guardrail` `Feature:` block has *both* branches
  (approved+rejected, safe+unsafe), not just the happy path — the single rule §8.3
  itself calls out as most load-bearing.
- **Subagent `shim-swap-auditor`** — targets this codebase's single most expensive
  documented failure mode directly (§4.6: "tests passing" only proves correctness
  against the shim, not the real package — AP-03 is the incident this caused). Its
  prompt encodes the full table of shim/real pairs in the codebase up front (chat
  client, embedding client, checkpointer, document store, cross-encoder, staging
  API client, LangSmith registry) rather than rediscovering them per review, and is
  explicit that "cannot verify, real package not installed here" is a valid,
  expected output rather than something to paper over.
- **Hook: YAML-validate-on-save** (`PostToolUse`, matcher `Write|Edit`) — parses
  `infra/*.yml`/`infra/*.yaml` on every edit. Directly motivated by a real incident:
  Feature 15's "Latent bug found and fixed" note documents `infra/docker-compose.yml`
  silently being invalid YAML (unquoted `${VAR:?message}` colons) for an unknown
  period, caught only by manual verification. Re-triggering that exact scenario
  during construction (see discipline point 5 below) confirmed the hook catches it
  immediately with the precise line/column.
- **Hook: check_env-consistency-on-save** (`PostToolUse`, matcher `Write|Edit`) —
  editing `infra/litellm_config.yaml` or `scripts/check_env.sh` runs
  `tests.gateway.test_check_env_credentials_match_config` immediately, rather than
  waiting for a full suite run to catch the two files drifting apart.
...
5. **Don't assume a scripting-language module is installed — pipe-testing catches
   this too, if you actually run it.** Building the YAML-validate hook, `python3 -c
   "import yaml"` failed in this environment: PyYAML is in `requirements.txt` but
   wasn't actually installed for the system `python3` (nor in the project's own
   `.venv`, which turned out to be an uninitialized `uv` venv without even `pip`
   present). macOS's built-in `ruby` ships with YAML (Psych) support with zero
   install, so the hook tries `ruby` first and falls back to PyYAML, then to a
   clean one-line skip message — never a raw traceback on every edit. The general
   lesson: a hook command that depends on a library must degrade gracefully across
   machines, not just work on the one it was written on. Pipe-testing against the
   *actual* target shell is what surfaces this — a command that looks obviously
   correct can still fail for an environment reason unrelated to its logic.
6. **A hook surfacing a pre-existing, unrelated problem is a feature, not a bug in
   the hook.** The `check_env-consistency-on-save` hook's own pipe-test revealed
   that `tests/gateway/test_check_env_credentials_match_config.py` already fails in
   this environment (same missing-PyYAML root cause as point 5) — i.e. `make
   test-local` has had 3 silent errors independent of anything built in this
   section. Don't fix an unrelated pre-existing gap while verifying a new hook;
   confirm the hook's own logic is correct (it correctly matched, dispatched, and
   reported the real failure) and flag the separate problem to the user rather than
   scope-creeping into fixing it unasked.
...
## 5. Claude Code Hooks: Sentinel's Automated Guardrails

Sentinel's `.claude/settings.json` defines four `PostToolUse` hooks that fire
automatically after every `Write` or `Edit` tool call in a Claude Code CLI session.
They enforce the project's structural rules and test discipline without requiring
any manual step — the check just happens.

**Hooks only run in Claude Code CLI** (`claude` command in terminal). They do not
run in Cowork or claude.ai. For the generic patterns and `settings.json` template
structure, see `GENERAL_ENGINEERING_PLAYBOOK.md` §5.

### 5.1 Hook 1 — Gateway Constraint Lint (ADR-006)

**Trigger:** any `Write` or `Edit` to `src/**/*.py` or `scripts/**/*.py`

**What it does:** runs `scripts/lint_gateway_usage.sh`, which greps for direct
`openai`, `anthropic`, or `langchain_openai` imports outside `src/gateway/`. Fails
with a non-zero exit if any are found.

**Why it exists:** ADR-006 mandates that every LLM and embedding call passes through
`src/gateway/client_factory.py`. Without automated enforcement, this invariant silently
erodes — a new node author imports `ChatOpenAI` directly and the gateway's fallback
chain, semantic cache, and trace injection are bypassed for that call. The hook makes
the violation immediately visible rather than discoverable only at code review or `make
lint` time.

**Sentinel-specific:** the path pattern and lint script are hardcoded to this
codebase. For a new project, replace the path glob and command with whatever enforces
that project's equivalent architectural constraint.

### 5.2 Hook 2 — Auto-Run Matching Node Test

**Trigger:** any `Write` or `Edit` to `src/graph/nodes/*.py`

**What it does:** derives the test file path by prepending `tests/graph/nodes/test_`
to the base filename, then runs `python3 -m unittest tests.graph.nodes.test_<name>`
if the test file exists. Skips `__init__.py`.

**Why it exists:** the graph node is the atomic unit of this codebase. Every node
has a corresponding test file. Without the hook, an edit to `router.py` requires
remembering to run `python3 -m unittest tests.graph.nodes.test_router` — a step that
is easy to skip when iterating quickly. With the hook, it runs automatically on every
save. Regressions surface in seconds, not at the end of a session.

**Path convention it assumes:** `src/graph/nodes/X.py` → `tests/graph/nodes/test_X.py`.
If you reorganize the graph structure, update the `case` glob and the test module
path in the hook command together.

### 5.3 Hook 3 — Infra YAML Syntax Validation

**Trigger:** any `Write` or `Edit` to `infra/*.yml` or `infra/*.yaml`

**What it does:** runs `yaml.safe_load()` (via Python or Ruby if available) against
the modified file and prints `YAML OK: <file>` or `YAML INVALID: <file>` with the
parse error.

**Why it exists:** invalid YAML in `infra/litellm_config.yaml` or
`infra/docker-compose.yml` causes `make up` to fail with a confusing error that is
not obviously a YAML syntax issue. The hook catches it at write time, before any
`make` command is run.

**Practically generic:** the only Sentinel-specific detail is the `infra/` path
prefix. This hook can be copied verbatim to any project; just widen or narrow the
path glob.

### 5.4 Hook 4 — Config Consistency Contract Test

**Trigger:** any `Write` or `Edit` to `infra/litellm_config.yaml` or
`scripts/check_env.sh`

**What it does:** runs
`python3 -m unittest tests.gateway.test_check_env_credentials_match_config`, which
asserts that the set of provider API keys declared in `litellm_config.yaml`'s
`model_list` exactly matches the set declared as `REQUIRED_VARS` in `check_env.sh`.

**Why it exists:** `litellm_config.yaml` and `check_env.sh` are a paired contract —
if a new provider is added to the model list without adding its key to the env check,
`make up` succeeds but `make smoke` fails mysteriously because the provider auth is
missing. The hook makes this drift impossible to miss: edit either file and the
contract test runs immediately.

**Pattern reuse:** replace `litellm_config.yaml`/`check_env.sh` with whatever paired
files your project must keep in sync, and replace the test command with the contract
test that validates their consistency. See `GENERAL_ENGINEERING_PLAYBOOK.md` §5.2,
Pattern 4 for the generic form.

### 5.5 The Full settings.json
    ...
    "PostToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "jq -r '.tool_input.file_path // .tool_response.filePath' | { read -r f; case \"$f\" in */src/*.py|src/*.py|*/scripts/*.py|scripts/*.py) bash scripts/lint_gateway_usage.sh ;; esac; }",
            "statusMessage": "Checking gateway-only usage (ADR-006)..."
          },
          {
            "type": "command",
            "command": "jq -r '.tool_input.file_path // .tool_response.filePath' | { read -r f; base=\"$(basename \"$f\" .py)\"; case \"$f\" in */src/graph/nodes/*.py|src/graph/nodes/*.py) if [ \"$base\" != \"__init__\" ] && [ -f \"tests/graph/nodes/test_${base}.py\" ]; then python3 -m unittest \"tests.graph.nodes.test_${base}\"; fi ;; esac; }",
            "statusMessage": "Running matching node test..."
          },
          {
            "type": "command",
            "command": "jq -r '.tool_input.file_path // .tool_response.filePath' | { read -r f; case \"$f\" in */infra/*.yml|infra/*.yml|*/infra/*.yaml|infra/*.yaml) if command -v ruby >/dev/null 2>&1; then ruby -ryaml -e \"YAML.load_file(ARGV[0])\" \"$f\" && echo \"YAML OK: $f\" || echo \"YAML INVALID: $f\"; elif python3 -c \"import yaml\" >/dev/null 2>&1; then python3 -c \"import sys, yaml; yaml.safe_load(open(sys.argv[1]))\" \"$f\" && echo \"YAML OK: $f\" || echo \"YAML INVALID: $f\"; else echo \"skip: no YAML parser available\"; fi ;; esac; }",
            "statusMessage": "Validating infra YAML syntax..."
          },
          {
            "type": "command",
            "command": "jq -r '.tool_input.file_path // .tool_response.filePath' | { read -r f; case \"$f\" in */infra/litellm_config.yaml|infra/litellm_config.yaml|*/scripts/check_env.sh|scripts/check_env.sh) python3 -m unittest tests.gateway.test_check_env_credentials_match_config ;; esac; }",
            "statusMessage": "Checking check_env.sh/litellm_config.yaml consistency..."
          }
        ]
      }
    ]
...
[ ] 6. Pipe-test every hook command against a synthetic stdin JSON payload (matching
       ...
       | .command'` — exit 0 and the expected command printed back. If a command
       depends on a library (YAML/JSON/etc. parser), verify it's actually importable
       in the target shell rather than assuming it — pipe-testing on the wrong
       interpreter/venv is how this playbook's own YAML-validate hook discovered
       PyYAML wasn't installed despite being in requirements.txt; it now tries a
       zero-dependency fallback (macOS's built-in `ruby -ryaml`) before PyYAML, then
       skips cleanly with one line rather than a raw traceback.
